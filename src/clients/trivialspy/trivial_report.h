...
void generateThreadOverviewReport(file_t file, per_thread_log_t *pt) {
    ...
    dr_fprintf(file, "\nTotal Expected Benifit: %.3lf (%ld benifit / %ld total cost)", 100.0*(double)totalSB/(double)totalCost, totalSB, totalCost);
    dr_fprintf(file, "\nTotal Branching Benifit: %.3lf (%ld benifit / %ld total cost)", 100.0*(double)totalBB/(double)totalCost, totalBB, totalCost);
...
void generateThreadDetailedReport(file_t file, per_thread_log_t *pt, std::vector<RedundancyData>& tmpList, int max_print, bool BB_order) {
    ...
    for(auto it=tmpList.begin(); it!=tmpList.end() && num_print<max_print; ++it, ++num_print) {
        ...
        dr_fprintf(file, "\n===============================\n");
        dr_fprintf(file, "Branching Benifit: %.3lf (%ld local BB / %ld total BB)\n",
        ...
        dr_fprintf(file, "Expected Benifit: %.3lf (%ld local EB / %ld EB)\n",
...
void generateThreadReport(per_thread_log_t *pt, uint64_t* bb_ref, file_t file, file_t gFile, int threshold, int max_print) {
    ...
    // sort by SB
    dr_fprintf(file, "\n------- Trivial Hotspots ordered by EB --------\n");
